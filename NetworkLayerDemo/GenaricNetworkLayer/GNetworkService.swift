...
//MARK: - NetworkService
typealias GHandler<T> = ([T]?, Error?) -> Void
...
protocol GNetworkServiceProtocol {
    func getDataByPublisher<T:Codable>(modelType:T.Type, url:String) -> AnyPublisher<[T],Error>
    func getDataByCompletion<T:Codable>(modelType:T.Type, url:String, completion: @escaping GHandler<T>)
    func getDataByAsync<T:Codable>(modelType:T.Type, url:String)async throws -> [T]?
}

class GNetworkService:GNetworkServiceProtocol {
